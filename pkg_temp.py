...
class Database:
    ...
    def store_package(self, package_name: str) -> str:
        print(f"INSERT INTO packages VALUES {package_name}")
...
def add_package(database):
    ...
    if not package_name:
        ...
        return
    if requests.get(f"{BASE_URL}/{package_name}/json", timeout=6).status_code != 200:
...
def update_database():
    ...
    for package_id, row in df_packages.iterrows():
        ret = requests.get(f"{BASE_URL}/{row['package_name']}/json", timeout=6)
